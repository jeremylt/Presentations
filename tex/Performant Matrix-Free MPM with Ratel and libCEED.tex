...
\title[Matrix-Free MPM]{Performant MPM Basis Operations for GPU Architectures} % The short title appears at the bottom of every slide, the full title is only on the title page
...
\date{11 Sept 2025} % Date, can be changed to a custom date
...
\item Note - MAGMA faster for non-tensor elements here\\
...
\item Conservative shared memory usage for FEM with tensor-product elements\\

~\\

\item Largely similar shared memory usage for MPM\\

~\\

\item MPM needs $Q \times Q$ \lstinline{atomicAdd} per 2D plane in transpose!\\
...
\begin{frame}
\begin{center}
\frametitle{Preconditioning ingredients}

\begin{itemize}

\item Fused kernels for diagonal/full assembly\\

~\\

\item FEM assembly can assemble {\color{applegreen}$D$} once and reuse\\

~\\

\item MPM assembly cannot exploit this same structure easily\\
...
\item Requires several operators from the user (or generated algorithmically)\\
